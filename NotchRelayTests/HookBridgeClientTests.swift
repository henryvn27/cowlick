...
final class HookBridgeClientTests: XCTestCase {
  ...
  func testRejectsInsecureRuntimeMetadata() throws {
    ...
    let metadataURL = home.appendingPathComponent(
      "Library/Application Support/NotchRelay/runtime.json")
  ...
  private func makeTemporaryHome() throws -> URL {
    let url = FileManager.default.temporaryDirectory.appendingPathComponent(
      "NotchRelayTests-\(UUID().uuidString)")
  ...
  private func makeSocketFixture(
    ...
    let home = try makeTemporaryHome()
    let runtimeDirectory = home.appendingPathComponent("Library/Application Support/NotchRelay")
    try FileManager.default.createDirectory(at: runtimeDirectory, withIntermediateDirectories: true)
    let socketPath = "/tmp/notchrelay-\(UUID().uuidString.prefix(8)).sock"
