...
enum HookCommand {
  ...
  static func run(arguments: [String]) -> Int32 {
    guard let command = arguments.dropFirst().first else {
      writeError("usage: notchrelay-hook <hook|ping|diagnostics|demo|version>")
      return 2
    }
    ...
    case "demo":
      guard let name = arguments.dropFirst(2).first else {
        writeError("usage: notchrelay-hook demo <working|approval|completed|failed>")
        return 2
      }
      return runDemo(name: name, client: client)
    case "version", "--version", "-v":
      FileHandle.standardOutput.write(Data("NotchRelay hook \(version)\n".utf8))
      return 0
    default:
  ...
  private static func writeError(_ message: String) {
    FileHandle.standardError.write(Data("notchrelay-hook: \(message)\n".utf8))
  }
}
