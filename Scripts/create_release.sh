...
version="${1:-1.0.0}"
profile="${NOTARYTOOL_PROFILE:-notchrelay-notary}"
...
app="$project_root/build/export-$version/NotchRelay.app"
...
(cd "$output" && shasum -a 256 "NotchRelay-$version.zip" "NotchRelay-$version.dmg" > checksums.txt)
