...
final class SessionStore {
  ...
  var activeSessionCount: Int {
    sessions.values.filter {
      switch $0.status {
      case .working, .awaitingApproval, .failed: true
  ...
  private func complete(_ event: BridgeEvent, projectName: String) {
    ...
    session.completionVisibleUntil =
      visibility.map { Date().addingTimeInterval($0) } ?? .distantFuture
